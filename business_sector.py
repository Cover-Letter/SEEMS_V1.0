...
#   
    ...
    def calculate_business_revenue(self, capital, model_parameters,risk_type,risk_effective,iteration_count = 0,D ={} ,HH_list= []):
        '''
        capital - household's own capital properties (factors of production) i.e. household_class.own_capital_properties
        risk_type - household's preference toward risks. True - risk aversion; False - risk appetite;
        ...
        if self.SectorName == 'Agriculture':
            ...
            '''
            if new_capital.CostDist_Farm != None and new_capital.CostDist_Farm >15:

                spec_labor_cost = self.LaborCost * (-(math.log(float(1 / math.log(new_capital.CostDist_Farm)))))
            else:
            ...
            if theoretical_max_farm > new_capital.av_farmland:
                ...
                Arealist = []
                for land_parcel in new_capital.land_properties_list:
                    Arealist.append(land_parcel.Shape_Area)
                max_farm = 0
                while (max_farm < theoretical_max_farm and Arealist is not None):
                    i = max(Arealist)
                    j = Arealist.index(i)
                    max_farm += i / 666.7
                    land_parcel2 = new_capital.land_properties_list[j]
                    land_parcel2.actual_farming = True
                    Arealist.pop(j)
                if max_farm > theoretical_max_farm:
                    max_farm = max_farm - i / 666.7
                    land_parcel2.actual_farming = False
            ...
            if iteration_count > 1:
                if new_capital.number in HH_list:
                    for i in D[new_capital.number]:
                        if i > 6:
                            #Animal Impact
                            revenue -= random.uniform(0.1,0.3) * revenue
