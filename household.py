...
@author: Liyan Xu; Hongmou Zhang; Yufeng Chen
'''
# import copy
import random
import math
...
class Household(object):
    ...
    def household_economy(self, policies, business_sectors, model_parameters,iteration_count,current_year):
        '''
        ...
        self.household_policy_decision(policies, business_sectors, model_parameters,iteration_count,current_year)
        ...
        self.own_capital_properties = self.household_business_revenue(self.own_capital_properties, 
                                                    business_sectors, model_parameters,iteration_count,current_year,risk_effective=True)
...
#         p_labor = math.exp(0.461 * self.own_capital_properties.labor) / (1 + math.exp(0.461 * self.own_capital_properties.labor))
    ...
    def household_policy_decision(self, policy_dict, business_sector_dict, model_parameters,iteration_count,current_year):
        ...
        if self.own_capital_properties.farm_to_forest != 0: # Has already joined the farm to forest program
            ...
            hyp_capital_1 = self.household_business_revenue(hyp_capital_1, business_sector_dict, model_parameters, iteration_count,current_year,risk_effective=False)
            ...
            hyp_capital_2 = self.household_business_revenue(hyp_capital_2, business_sector_dict, model_parameters, iteration_count,current_year, risk_effective=False)
...
#                         self.own_capital_properties.land_properties_list.remove(farmland_parcel)
    ...
    def household_business_revenue(self, hh_capital,business_sector_dict, model_parameters,iteration_count,current_year,risk_effective):
        '''
        ...
        self.get_rank_available_business(hh_capital,self.own_av_business_sectors, model_parameters,iteration_count,current_year)
        
        hh_capital = self.do_business(hh_capital, self.own_av_business_sectors, model_parameters,iteration_count,current_year, risk_effective)
    ...
    def get_rank_available_business(self,hh_capital, business_list, model_parameters,iteration_count,current_year):
        '''
        ...
        if self.hh_preference_type == 1:
            ...
            for sector in business_list:
                profit = sector.calculate_business_revenue(hh_capital, model_parameters, risk_type=True,
                                                            risk_effective=False).cash - hh_capital.cash
            ...
            for sector in business_list:
                labor_cost = sector.calculate_business_revenue(hh_capital, model_parameters, risk_type=True,
                                                                risk_effective=False).labor_cost - hh_capital.labor_cost
            ...
            # Get the hypothetical profit of each sector if entered
            for sector in business_list:
                profit = sector.calculate_business_revenue(hh_capital, model_parameters, risk_type=False,
                                                            risk_effective=False).cash - hh_capital.cash
                labor_cost = sector.calculate_business_revenue(hh_capital, model_parameters, risk_type=True,
                                                                risk_effective=False).labor_cost - hh_capital.labor_cost
                if labor_cost != None and labor_cost != 0:
                    temp_sectors_list.append((profit/labor_cost, sector))
            # Rank the sectors by profit descendedly
            temp_sectors_list.sort(reverse=True)
            ...
            # Get the hypothetical labor cost of each sector if entered
            for sector in business_list:
                labor_cost = sector.calculate_business_revenue(hh_capital, model_parameters, risk_type=False,
                                                                risk_effective=False).labor_cost - hh_capital.labor_cost
                temp_sectors_list.append((labor_cost, sector))
    ...
    def do_business(self, hh_capital, business_list, model_parameters,iteration_count,current_year, risk_effective):
        '''
        risk_effective - whether the random risk factor takes effect in the calculation. True - real world; False: hypothetical.        
        '''
        ...
        if iteration_count > 0:
            InTable = "C:/WolongRun/Results_Output/land/zonal" + '_' + str(iteration_count - 1) + '.dbf'
            nTable = DBF(InTable)
            c = dict()
            Hlist = list()
            for record in nTable:
                c.setdefault(record['HID'], []).append(record['MEAN'])
                Hlist.append(record['HID'])
        self.own_current_sectors = list()
        ...
        if self.hh_preference_type == 1 or self.hh_preference_type == 3:
            ...
            if  iteration_count > 0:
                for sector in self.own_av_business_sectors:
                    hh_capital = BusinessSector.calculate_business_revenue(sector,hh_capital, model_parameters,self.hh_risk_type,risk_effective,iteration_count,c, Hlist)
            ...
            if iteration_count > 0:
                for sector in self.own_av_business_sectors:
                    if hh_capital.get_total_business_income() + hh_capital.compensational_revenues >= self.get_min_living_cost(
                            model_parameters):
