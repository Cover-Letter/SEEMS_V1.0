...
#         self.count2 = 0
    ...
    def society_step_go(self, start_year, iteration_count):
        ...
        self.economic_activities(iteration_count,self.current_year)
            
        # The land sub-model.
        self.land_update(iteration_count)
           
        # The energy (and carbon, and habitat) sub-model.
        self.energy_update(start_year, iteration_count)
...
#                 relation = '5_00_0'
    ...
    def economic_activities(self,iteration_count,current_year):
        
        for HID in self.hh_dict:
            
            self.hh_dict[HID].household_economy(self.policy_dict, self.business_sector_dict, self.model_parameters_dict,iteration_count,current_year)
    ...
    def land_update(self,iteration_count):
        '''
        ...
        for ParcelID in self.land_dict:
            ...
            self.land_dict[ParcelID].land_step_go(self, iteration_count)
        ...
        for ParcelID in self.land_dict:
            self.land_dict[ParcelID].land_neibourhood(iteration_count)
            break



    def energy_update(self, start_year, iteration_count):
        '''
        To be elaborated.
        '''
        pass
        # this_year_firewood_need = 0
        # for HID in self.hh_dict:
        #     Energy.energy_step_go(self.hh_dict[HID].energy, self.hh_dict[HID], self.model_parameters_dict)
        #     # Get the statistics
        #     if self.hh_dict[HID].is_exist == 1:
        #         this_year_firewood_need += self.hh_dict[HID].energy.firewood_consumption
        # each_group_collect = this_year_firewood_need / 30
        #
        # thisyear_Energy = Energy()
        # thisyear_Energy.get_firewood_collection_area(each_group_collect,self.current_year,iteration_count)
